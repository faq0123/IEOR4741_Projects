...
static void generate_ticks(std::vector<Quote>& out, uint32_t n, uint32_t seed) {
    ...
    for (uint32_t i = 0; i < n; ++i) {
        ...
        double sprd = rng.uniform(0.0005, 0.02);     // spread 0.05–2 cents
        double bq   = rng.uniform(100.0, 5000.0);    // bid size
        double aq   = rng.uniform(100.0, 5000.0);    // ask size
        out[i] = Quote{midp - 0.5 * sprd, midp + 0.5 * sprd, bq, aq};
    }
}

// Benchmark runner: prevents dead code elimination by accumulating into sink
template<typename F>
